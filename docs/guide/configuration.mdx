...
Choose the bundler backend. Rollup is the default and fully-supported path; the
experimental [Rolldown](https://rolldown.rs) backend is opt-in:
...
See the [bundler option](/docs/options/bundler) for the full Rolldown caveats.
...
See the [declaration option](#declaration) above for the meaning of each value.
