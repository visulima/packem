...
When `builtinNodeModules: true` is set, the plugin transforms Node.js built-in modules to use `globalThis.process.getBuiltinModule()`:
...
This provides even better performance for built-in modules.
