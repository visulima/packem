...
// Lazy on purpose: `import.meta.url` is only read on the first CJS `require`, not at module
// evaluation. Bundles that leave `import.meta.url` undefined (Cloudflare Workers/workerd, Vite
// production builds) would otherwise throw at load — a hard boot failure — even when no CJS dep
...
- [ESM Migration Guide](/docs/guide/migration/rollup)
