...
Packem is a bundler for Node.js and TypeScript libraries. It reads your `package.json` exports, picks the right entry points, and produces the ESM and CJS output your `package.json` declares — with TypeScript declarations, CSS processing, and tree shaking included.

Under the hood it uses Rollup for bundling and lets you swap in the compiler that fits your project: esbuild, swc, OXC, sucrase, or the official TypeScript compiler. An experimental [Rolldown](https://rolldown.rs) backend is also selectable via the [`bundler`](/docs/options/bundler) option.
...
- **Multiple output formats** — ESM and CJS in a single build.
