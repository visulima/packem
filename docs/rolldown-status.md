...
| CI job                   | Advisory — runs on every PR touching packages but is **excluded from `test-required-check`**. Rationale: "known cache/ordering flakiness; surface regressions without gating merges until it has proven stable (see plans/001)."                                                                                                                                   | `.github/workflows/test.yml:150-196`                                                           |
