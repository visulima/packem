...
Packem validates that your `package.json` `exports` map matches the formats it
produces. This runs automatically; configure it through the `validation` option
rather than a manual hook:
...
See the [validation docs](/docs/guide/package-json) for the full set of checks.
...
`dependencies`, `peerDependencies`, and Node built-ins are externalized by
default. Use the top-level `externals` array to force-externalize extra
packages, and `rollup.resolveExternals` to change classification:
...
See the [externals option](/docs/options/externals) for the full classification
controls (`deps`, `peerDeps`, `exclude`, …).
