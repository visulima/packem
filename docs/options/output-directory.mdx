...
description: Configure where Packem writes your built files and assets
---
...
### Separate directories per format

`rollup.output` is a single object that Packem spreads into every output it generates, so
it cannot describe one directory per format. Point each `exports` condition at the
directory you want instead — the build follows the published paths:
...
- [Assets](/docs/guide/introduction) - Handle static assets
- [Clean](/docs/options/cleaning) - Clean output directories