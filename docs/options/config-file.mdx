...
<Tabs items={['CLI Flag', 'Environment Variable']}>
  <Tab value="CLI Flag">
    ```bash
...
export default defineConfig({
  ...
  rollup: {
    output: {
      // TypeScript autocompletes Rollup's own options here. Note that these are
      // passed straight through: Packem itself only produces ESM and CJS, and
      // overriding `format` does not change output file names — see
      // /docs/options/output-format#browser-globals
      sourcemap: true
    }
  }
})
