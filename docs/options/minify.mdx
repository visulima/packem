...
esbuild can preserve original `function`/`class` names through minification (so `fn.name` and stack traces stay readable). Packem leaves this **off by default** — matching esbuild's own default — and lets you opt in:
