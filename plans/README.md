...
| 015  | Route packem's DTS through rolldown natively (012 GO path, item 6) | P2 | L | MED-HIGH | 014 | DONE — branch `advisor/015-rolldown-native-dts` (PR open; unmerged). `getRolldownDtsOptions` + region-strip plugin added; build-types/watch branch on `isRolldown`; rollup pull-in + "falls back to rollup for DTS" hint/info-log removed. Verified: build + `lint:types` clean, all touched files eslint-clean, rollup integration 502 pass / 0 snap changes, typescript both backends green (new Step-6 `no //#region` regression test passes), watch 4/4 both backends. `test:rolldown` = 465 pass / 8 fail — the 8 fails (externals 2, css 5, typescript-deeks 1) are byte-identical to the alpha baseline (pre-existing path-sensitive/stale-snapshot debt), so zero new failures. Reviewer at merge: tick `docs/rolldown-status.md` criterion 2 + refresh its section-1 DTS rows and section-3 wizard-hint row; the 3 path-sensitive rolldown snapshot files still need regen on the canonical checkout (regenerating in a worktree bakes worktree paths — STOP condition) |
