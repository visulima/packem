...
describe("css Sourcemap Utilities", () => {
    it("should correctly handle inline sourcemaps", () => {
        expect.assertions(3);
        ...
        expect(getMap(code)).toBeUndefined();

        // eslint-disable-next-line no-secrets/no-secrets
        const withBlock = `${code}/*# sourceMappingURL=data:application/json;base64,e1RISVM6SVNBU09VUkNFTUFQU0lNVUxBVElPTn0= */`;

        expect(getMap(withBlock)).toBe("{THIS:ISASOURCEMAPSIMULATION}");

        // eslint-disable-next-line no-secrets/no-secrets
        const withInline = `${code}//# sourceMappingURL=data:application/json;base64,e1RISVM6SVNBU09VUkNFTUFQU0lNVUxBVElPTn0=`;

        expect(getMap(withInline)).toBe("{THIS:ISASOURCEMAPSIMULATION}");
    });

    it("should correctly handle file-based sourcemaps", () => {
        expect.assertions(3);

        // eslint-disable-next-line no-secrets/no-secrets
        const code = ".foo {color: red;}/*# sourceMappingURL=fixture.css.map */";

        expect(() => getMap(code)).toThrowErrorMatchingSnapshot();
        expect(getMap(code, "this/is/nonexistant/path.css")).toBeUndefined();
        expect(getMap(code, join(fixturePath, "utils/pointless.css"))).toBe("{THIS:ISASOURCEMAPSIMULATION}");
    });
