...
export const filename_to_dts = (id: string): string => id.replace(RE_VUE, ".vue.ts").replace(RE_TS, ".d.$1ts").replace(RE_JS, ".d.$1ts").replace(RE_JSON, ".json.d.ts");
export const filename_dts_to = (id: string, extension: "js" | "ts"): string => id.replace(RE_DTS, `.$1${extension}`);

export const resolveTemplateFn = (function_: ((chunk: PreRenderedChunk) => string) | string, chunk: PreRenderedChunk): string => typeof function_ === "function" ? function_(chunk) : function_;
