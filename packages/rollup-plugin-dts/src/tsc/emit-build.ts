...
import type { ParsedProject, SourceFileToProjectMap, TscContext } from "./context.ts";
...
function getOrBuildProjects(context: TscContext, fsSystem: ts.System, tsconfig: string, force: boolean, sourcemap: boolean): SourceFileToProjectMap {
 ...
 */
function buildProjects(fsSystem: ts.System, tsconfig: string, force: boolean, sourcemap: boolean): SourceFileToProjectMap {
 ...
 */
function collectProjectGraph(rootTsconfigPath: string, fsSystem: ts.System, force: boolean, sourcemap: boolean): ParsedProject[] {
...
function parseTsconfig(tsconfigPath: string, fsSystem: ts.System): ts.ParsedCommandLine | undefined {
    ...
    if (diagnostics.length > 0) {
        throw new Error(`[rolldown-plugin-dts] Unable to read ${tsconfigPath}: ${ts.formatDiagnostics(diagnostics, formatHost)}`);
    }
...
function patchCompilerOptions(
    ...
    if (noEmit === true) {
        ...
        if (shouldPrintWarning) {
            console.warn(`[rolldown-plugin-dts] ${extraOptions.tsconfigPath} has "noEmit" set to true. Please set it to false to generate declaration files.`);
        }
    }
