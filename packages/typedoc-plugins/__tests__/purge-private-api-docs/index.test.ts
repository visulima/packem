...
describe("typedoc-plugins/purge-private-api-docs", function () {
    ...
    before(async () => {
        ...
        typeDoc.bootstrap({
            logLevel: "Error",
            entryPoints: files,
            modifierTags: ["@internal"],
            plugin: [plugins["typedoc-plugin-module-fixer"], plugins["typedoc-plugin-purge-private-api-docs"]],
    ...
    describe("private packages with the `@publicApi` annotation - extending a public package", () => {
        it("should keep inherited public reflections", () => {
            const extendPublicCollection = conversionResult.getChildByName(["private-package-public-api/extendpubliccollection", "ExtendPublicCollection"]);

            const publicCollection = conversionResult.getChildByName(["public-package/publiccollection", "PublicCollection"]);

            expect(publicCollection).to.not.equal(undefined);
            expect(extendPublicCollection).to.not.equal(undefined);

            const publicFields = ["publicValue", "declarePublicValue", "getPublicFunction"];
        ...
        it("should keep inherited protected reflections", () => {
            ...
            const protectedFields = ["protectedValue", "declareProtectedValue", "getProtectedFunction"];
        ...
        it("should keep inherited internal reflections (`@internal`)", () => {
            ...
            const internalFields = ["_internalValue", "_declareInternalValue", "_getInternalFunction"];
        ...
        it("should not keep inherited private reflections (cannot inherit private fields)", () => {
            ...
            const privateFields = ["privateValue", "declarePrivateValue", "getPrivateFunction"];
    ...
    describe("private packages with the `@publicApi` annotation - extending a private package", () => {
        it("should keep inherited public reflections", () => {
            const extendPrivateCollection = conversionResult.getChildByName(["private-package-public-api/extendprivatecollection", "ExtendPrivateCollection"]);
