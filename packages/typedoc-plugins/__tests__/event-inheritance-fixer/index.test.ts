...
describe("typedoc-plugins/event-inheritance-fixer", function () {
    ...
    const PLUGINS = [plugins["typedoc-plugin-tag-event"], plugins["typedoc-plugin-event-inheritance-fixer"]];
    ...
    before(async () => {
        ...
        events = conversionResult.getReflectionsByKind(TypeDoc.ReflectionKind.All).filter((child) => child.kindString === "Event");
    ...
    it("should clone event comment in the inherited classes and interfaces", () => {
        ...
        for (const event of eventsToCheck) {
            ...
            expect(event.comment.summary[0])
                .to.have.property("text")
                .to.match(/Event 1 from (class|interface) A./);
    ...
    it("should copy event parameters and all their properties in the inherited classes and interfaces", () => {
        ...
        for (const event of [baseEventClassA, inheritedEventClassB, inheritedEventClassC]) {
            ...
            expect(event.typeParameters[2].comment.summary[1]).to.have.property("text", "module:utils/object~Object description");
