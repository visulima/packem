    ...
    it("should split shared module into one chunk layer", async () => {
        await createIntegrationTest(
            ...
            async ({ distDir }) => {
                ...
                const sharedUtilModuleFiles = files.filter((file) => file.startsWith("util"));
                const entryModuleFiles = files.filter((file) => !file.includes("shared-"));
                const appContextSharedFiles = files.filter(
                    (file) => file.startsWith("app-context-shared-") && readFileSync(path.join(distDir, file), "utf-8").includes("use client"),
                );
                ...
                const indexFileMatcher = (content: string) => {
                    return !content.includes("common:shared") && content.includes("sharedApi");
                };

                const anotherFileMatcher = (content: string) => {
                    return !content.includes("common:shared") && content.includes("anotherSharedApi");
                };

                const appContextSharedFileMatcher = (content: string) => {
                    return content.includes("use client") && content.includes("createContext");
                };
