...
const __packemNativeDirectory = (() => {
    const { join } = __packemNodeRequire("node:path");
    const { tmpdir } = __packemNodeRequire("node:os");

    return process.env[${JSON.stringify(NATIVES_DIR_ENVIRONMENT)}] || join(tmpdir(), "packem-natives", ${JSON.stringify(buildId)});
})();
...
const __packemExtractNative = (name) => {
    ...
    if (!fs.existsSync(target)) {
        fs.mkdirSync(__packemNativeDirectory, { recursive: true });
