...
const createOrUpdateEntry = (
    ...
    for (const runtimeExportConvention of RUNTIME_EXPORT_CONVENTIONS) {
        if (
            output.file.includes(`.${runtimeExportConvention}.`)
            || output.subKey === runtimeExportConvention
        ) {
    ...
    const aliasName = output.file
        .replace(extname(output.file), "")
        .replace(
            new RegExp(`^./${context.options.outDir.replace(/^\.\//, "")}/`),
            "",
        );

    if (
        SPECIAL_EXPORT_CONVENTIONS.has(output.subKey as string)
...
const validateIfTypescriptIsInstalled = (
    context: BuildContext<InternalBuildOptions>,
): void => {
    if (
...
const inferEntries = (
    ...
    if (packageJson.bin) {
        ...
        for (const file of binaries) {
            ...
            if (inferredType && inferredType !== packageType) {
                throw new Error(
                    `Exported file "${file}" has an extension that does not match the package.json type "${packageJson.type ?? "commonjs"}".`,
                );
            }
    ...
    for (const output of outputs) {
        ...
        if (
            outputExtension !== ""
            && !DEFAULT_EXTENSIONS.includes(outputExtension)
        ) {
            continue;
        }

        if (
            context.options.declaration === undefined
            && (output.key === "types" || output.subKey === "types")
        ) {
            context.options.declaration = output.file.includes(".d.ts")
                ? "compatible"
                : true;
        }
        ...
        if (output.file.includes("/*") && output.key === "exports") {
            ...
            const sourceSlugWithoutExtension = sourceSlug.replace(
                /^(.+?)\.[^.]*$/,
                "$1",
            );
            const SPECIAL_SOURCE_RE = new RegExp(
                beforeSourceRegex
                + sourceSlugWithoutExtension.replace("*", "(.*)")
                + fileExtensionRegex,
            );
            ...
            if (inputs.length === 0) {
                warnings.push(
                    `Could not find entrypoint for \`${output.file}\``,
                );
        ...
        const SOURCE_RE = new RegExp(
            beforeSourceRegex + sourceSlug + fileExtensionRegex,
        );
        ...
        if (input === undefined) {
            if (
                !isAccessibleSync(resolve(context.options.rootDir, output.file))
            ) {
                warnings.push(
                    `Could not find entrypoint for \`${output.file}\``,
                );
        ...
        const inputWithoutExtension = toNamespacedPath(
            input.replace(ENDING_REGEX, ""),
        );

        if (
            isAccessibleSync(`${inputWithoutExtension}.cts`)
            && isAccessibleSync(`${inputWithoutExtension}.mts`)
        ) {
            createOrUpdateEntry(
                entries,
                `${inputWithoutExtension}.cts`,
                isDirectory,
                outputSlug,
                { ...output, type: "cjs" },
                context,
