...
describe("packem generate-license", () => {
    ...
    it("should bundle the license file placeholder if a dev-required package is bundled", async () => {
        ...
        writeFileSync(
            `${temporaryDirectoryPath}/node_modules/dep/index.js`,
            `export default "dep"`,
        );
        writeJsonSync(
            `${temporaryDirectoryPath}/node_modules/dep/package.json`,
            { name: "dep", version: "1.0.0" },
        );
        writeFileSync(
            `${temporaryDirectoryPath}/node_modules/dev/LICENSE`,
            `The MIT License (MIT) dep`,
        );

        writeFileSync(
            `${temporaryDirectoryPath}/node_modules/dev-dep/index.js`,
            `export default "dev-dep"`,
        );
        writeJsonSync(
            `${temporaryDirectoryPath}/node_modules/dev-dep/package.json`,
            { name: "dev-dep", version: "1.0.0" },
        );
        writeFileSync(
...
<!-- /TYPE_DEPENDENCIES -->`);
        ...
        const licenseContent2 = readFileSync(
            `${temporaryDirectoryPath}/LICENSE.md`,
        );
